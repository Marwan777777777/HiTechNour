...
// This lets the profile workflow evolve without touching the tested check-in logic.
(function () {
    ...
    section.innerHTML = `
      ...
      <label><span>New password</span>
        <div style="position:relative">
          ...
          <button type="button" class="icon-button" data-password-toggle="new-password" style="position:absolute;right:6px;top:50%;transform:translateY(-50%)" aria-label="Show password">👁</button>
        </div>
      </label>
      <label><span>Confirm new password</span>
        <div style="position:relative">
          <input type="password" id="confirm-password" autocomplete="new-password" minlength="8" />
          <button type="button" class="icon-button" data-password-toggle="confirm-password" style="position:absolute;right:6px;top:50%;transform:translateY(-50%)" aria-label="Show password">👁</button>
        </div>
      </label>
      <div class="button-row" style="margin-top:.75rem;gap:.6rem;flex-wrap:wrap">
        <button type="button" id="change-password-btn" class="secondary-button">Change password</button>
        <button type="button" id="enable-notifications-btn" class="secondary-button">Enable notifications</button>
      </div>
  ...
  async function enableNotifications() {
    ...
    window.HTNNotify.showLocal("HiTechNour notifications enabled", "You will receive new alerts while the app is installed/active.", "htn-test");
